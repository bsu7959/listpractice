...
<%
      ...
      <%
         ...
         String up_memIdx = (String)session.getAttribute("useridx");
         ...
         try{
            ...
            if(conn != null){
               sql = "select up_memIdx from tb_boardup where up_memIdx=? and up_boardIdx=?";
               pstmt = conn.prepareStatement(sql);
               pstmt.setString(1,up_memIdx);
               pstmt.setString(2,up_boardIdx);
               rs = pstmt.executeQuery();
               if(!rs.next()) {
	               sql = "insert into tb_boardup(up_memIdx, up_boardIdx) values(?,?)";
	               pstmt = conn.prepareStatement(sql);
	               pstmt.setString(1,up_memIdx);
	               pstmt.setString(2,up_boardIdx);
	               pstmt.executeUpdate(); 
               }
               sql = "select up_memIdx from tb_boardup where up_boardIdx=?";
               pstmt = conn.prepareStatement(sql);
